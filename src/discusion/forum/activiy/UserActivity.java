...
public class UserActivity {
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		else if (user.getUserRole() == UserRole.MODERATOR) {	// checking if the current user is a moderator
			if (question.getUser().getUserRole() == UserRole.USER)	// checking if the question asked by the user is having USER as a role
				questionService.deleteQuestion(question);
			else if (user == question.getUser())	// checking if the current user is same as the user who asked the question
				questionService.deleteQuestion(question);
			else
		...
		else if (user.getUserRole() == UserRole.USER) {		// checking if the current user is regular user
			if (user == question.getUser())		// checking if the current user is same as the user who asked the question
				questionService.deleteQuestion(question);
			else
				System.out.println("You are not authorised to delete this question");
		}
